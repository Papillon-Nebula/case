...
path = "ML/case/datas/risk_factors_cervical_cancer.csv"  # 数据文件路径
# ML\案例\datas\risk_factors_cervical_cancer.csv
# ML\case\datas\risk_factors_cervical_cancer.csv
...
#异常值处理
X = X.replace("?",np.NaN)
imputer = SimpleImputer(missing_values=np.nan, strategy='mean')     #使用Imputer给定缺省值，默认以列的mean
...
plt.savefig("ML\case\随机森林多目标属性分类ROC曲线.png")
plt.show()
